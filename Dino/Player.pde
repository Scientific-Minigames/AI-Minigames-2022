class Player {
  ...
  int genomeInputs = 7; //INPUT
  int genomeNetworkInputs = 3;
  int genomeOutputs = 3;

  float[] networkVision = new float[genomeNetworkInputs];
  ...
  Player() {
    brain = new Genome(genomeNetworkInputs, genomeOutputs);
  }
...
//what could this do????
  ...
  void look() {
    ...
    networkVision[0] = vision[0];
    networkVision[1] = vision[3];
    networkVision[2] = vision[4];
  }
  ...
  void think() {
    ...
    decision = brain.feedForward(networkVision);
